  ...
  <div class="log">
    ...
    <div class="ng-scope">
      <form @submit.prevent="submit">
        <div class="zhBox">
          <ul>
            <li><input type="text" v-model="account_number.account" placeholder="账号"/></li>
            <li><input type="text" v-model="account_number.password" placeholder="密码"/></li>
            <li><input type="text" v-model="account_number.two_password" placeholder="确认密码"/></li>
          </ul>
          <div class="zddl">
          </div>
          <div class="logBox" @click="submit">
...
export default {
  ...
  methods: {
    submit () {
      ...
      console.log(user)
      if (user) {
        user.push(this.account_number)
        localStorage.setItem('hammer_user', JSON.stringify(user))
        alert('注册成功')
        this.$router.replace('/login')
      } else {
        alert('注册成功')
        localStorage.setItem('hammer_user', JSON.stringify([this.account_number]))
        this.$router.replace('/login')
      }
    }
  },
...
<style scoped>
  ...
  .ng-scope ul li:last-child{
    border: 0px;
  }
  .ng-scope ul li input{
    width: 100%;
    height: 100%;
    border: 0px;
    font-size: 28px;
